...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        def minValue(state,agentIndex, depth):
            ...
            for action in state.getLegalActions(agentIndex):
                successor = state.generateSuccessor(agentIndex, action)
                val = minimax(successor, nextAgentIndex, depth)
                worstVal = min(val,worstVal)
            return worstVal
        bestAction = None
        bestVal = -float('inf')
        for action in gameState.getLegalActions(0):
            successor = gameState.generateSuccessor(0, action)
            val = minimax(successor,1,0)
            if(val>bestVal):
                bestVal = val
                bestAction = action
        return bestAction
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        def expectiminimax(state,agentIndex,depth):
            if state.isWin() or state.isLose():
                return self.evaluationFunction(state)
            if agentIndex >= state.getNumAgents():
                agentIndex = 0
            ...
            if agentIndex == 0:
                ...
                return chanceValue(state, agentIndex,depth) 
        def maxValue(state, agentIndex, depth):
            bestVal = -float('inf')
            for action in state.getLegalActions(agentIndex):
                successor = state.generateSuccessor(agentIndex, action)
                val = expectiminimax(successor,1,depth)
                bestVal = max(val,bestVal)
            return bestVal
            
        def chanceValue(state,agentIndex, depth):
            numChance = 0 
            sumChance = 0
            for action in state.getLegalActions(agentIndex):
                successor = state.generateSuccessor(agentIndex, action)
                val = expectiminimax(successor, agentIndex + 1, depth)
                numChance += 1
                sumChance += val
            if(numChance == 0):
                return self.evaluationFunction(state)
            return sumChance/numChance
            
        bestAction = None
        bestVal = -float('inf')
        for action in gameState.getLegalActions(0):
            successor = gameState.generateSuccessor(0, action)
            val = expectiminimax(successor,1,0)
            if(val>bestVal):
                bestVal = val
                bestAction = action
        return bestAction
